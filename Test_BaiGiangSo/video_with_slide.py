...
# Thư mục chứa các video avatar đã tạo (ví dụ: slide_1.mp4, slide_2.mp4)
...
# Khoảng cách lề (tính bằng pixel)pyt
MARGIN = 40
...
def process_all_videos():
    ...
    try:
        ...
        print("❌ Lỗi: Tên file trong thư mục 'videos' không đúng định dạng 'slide_NUMBER.mp4'.")
        return

    for video_filename in avatar_videos:
        # Tìm ảnh slide tương ứng (ví dụ: video "slide_1.mp4" sẽ khớp với ảnh "Slide1.JPG")
        try:
            slide_number = video_filename.split('_')[1].split('.')[0]
        except (IndexError, ValueError):
